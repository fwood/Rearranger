function[jumps] = note_movement(S)
% returns vector of sums of all possible non-rest note movements (95 x 1)
[~,B] = size(S);
...
for i=1:5
    ...
    for j=1:95
        jumps(j) = jumps(j) + sum(row == j);
    end
end
% return the vector of total counts for each of the 95 possible
% note-to-note movement jumps.  This will likely have very heavy weight at index
